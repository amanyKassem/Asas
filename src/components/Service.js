...
class Service extends Component {
    ...
    static navigationOptions = () => ({
        drawerLabel: 'اتفاقية الخدمة',
        drawerIcon: ( <Icon style={{ fontSize: 18,top:2, color: '#1da1f2' }} type={'FontAwesome'} name={'shield'}/> )
    })

    componentWillMount() {
        axios.get(CONST.url + 'agreaments').then(response => {
            this.setState({ agree: response.data.data, status: response.data.value })
        });
    }
    renderLoader(){
        ...
        return (
            ...
            <Container >
                <Header style={{ backgroundColor: "#fff" , alignItems: 'center', paddingTop: 25, height: 80, justifyContent: 'center'}}>
                    ...
                    <Text style={{color:'#797979' , alignItems: 'center', justifyContent: 'center' , fontFamily:"BoldFont" , fontSize:17}}>اتفاقية الخدمة</Text>
                    </Body>
                    <Right style={{flex:0}}>
                ...
                <Content style={{ padding: 20}}>
                    ...
                    <View style={{ flexDirection:'column' , alignItems:'center' , justifyContent:'center' , marginTop:70 }}>
                        ...
                        <Text style={{fontFamily:'RegularFont' , fontSize:15 , color:'#797979' , textAlign:'center'}} >{this.state.agree}</Text>
                ...
                <FooterSection routeName={'service'} navigation={this.props.navigation}/>
            </Container>
